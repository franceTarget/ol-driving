...
// import { getAuthSetting, saveImageToPhotosAlbum, equal } from "../../../static/util";
...
export default class QyPoster extends Component {
  ...
  // 执行绘制
  startPaint = () => {
    // console.log('[painter]:我要开始画了！！！我的id是：',this.canvasId)
    // 如果palette模板为空 则return
    if (this.isEmpty(this.props.palette)) {
      // console.log('[painter]:palette是空的，老子不画了！！',this.props.palette)
      return;
    }
    ...
    this.downloadImages().then((palette = any) => {
      ...
      if (this.props.widthPixels) {
        // 如果重新设置过像素宽度，则重新设置比例
        this.setStringPrototype(screenK, this.props.widthPixels / this.canvasWidthInPx);
  ...
  getImageInfo = (filePath) => {
    Taro.getImageInfo({
      ...
      success: (infoRes) => {
        ...
        if (
          Math.abs(
            (infoRes.width * this.canvasHeightInPx - this.canvasWidthInPx * infoRes.height) /
  ...
  render() {
    return <Canvas canvasId={this.canvasId} style={`${this.state.painterStyle}${this.props.customStyle}`} />;
  }
}
