...
export default class Painter {
  ...
  _doLine(view, width, height) {
    ...
    if (borderStyle === 'dashed') {
      ...
      this.ctx.beginPath();
      this.ctx.moveTo(30, this.yLine);
      this.ctx.lineTo(borderWidth, this.yLine);
      this.ctx.stroke();
      this.yLine += 20;
    }
  }
  ...
  _fillAbsText(view) {
    ...
    if (view.id) {
      ...
      for (let i = 0; i < textArray.length; ++i) {
        textWidth =
          this.ctx.measureText(textArray[i]).width > textWidth ? this.ctx.measureText(textArray[i]).width : textWidth;
      }
    ...
    for (let j = 0; j < textArray.length; ++j) {
      ...
      for (let i = 0; i < linesArray[j]; ++i) {
        ...
        while (
          ...
          (width - measuredWith > view.css.fontSize.toPx() || measuredWith > width)
        ) {
